...
class FDW<K, V> extends FD<K, V> {
  protected static final VarHandle WRITE_TIME;

  static {
    MethodHandles.Lookup lookup = MethodHandles.lookup();
    try {
      WRITE_TIME = lookup.findVarHandle(FDW.class, NodeFactory.WRITE_TIME, long.class);
  ...
  public Node<K, V> newNode(
      ...
      int weight,
      long now) {
    return new FDW<>(key, keyReferenceQueue, value, valueReferenceQueue, weight, now);
  }

  public Node<K, V> newNode(
      Object keyReference, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
    return new FDW<>(keyReference, value, valueReferenceQueue, weight, now);
  }
}
