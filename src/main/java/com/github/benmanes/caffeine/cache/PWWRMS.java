...
final class PWWRMS<K, V> extends PWWR<K, V> {
  int queueType;
  ...
  public Node<K, V> newNode(
      ...
      int weight,
      long now) {
    return new PWWRMS<>(key, keyReferenceQueue, value, valueReferenceQueue, weight, now);
  }

  public Node<K, V> newNode(
      Object keyReference, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
    return new PWWRMS<>(keyReference, value, valueReferenceQueue, weight, now);
  }
}
