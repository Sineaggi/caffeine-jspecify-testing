...
final class PDAMS<K, V> extends PDA<K, V> {
  ...
  PDAMS(Object keyReference, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
  ...
  public Node<K, V> newNode(
      ...
      int weight,
      long now) {
    return new PDAMS<>(key, keyReferenceQueue, value, valueReferenceQueue, weight, now);
  }

  public Node<K, V> newNode(
      Object keyReference, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
    return new PDAMS<>(keyReference, value, valueReferenceQueue, weight, now);
  }
}
