...
final class FSRMW<K, V> extends FSR<K, V> {
  int queueType;
  ...
  public Node<K, V> newNode(
      ...
      int weight,
      long now) {
    return new FSRMW<>(key, keyReferenceQueue, value, valueReferenceQueue, weight, now);
  }

  public Node<K, V> newNode(
      Object keyReference, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
    return new FSRMW<>(keyReference, value, valueReferenceQueue, weight, now);
  }
}
