...
final class PDRMS<K, V> extends PDR<K, V> {
  int queueType;
  ...
  public Node<K, V> newNode(
      ...
      int weight,
      long now) {
    return new PDRMS<>(key, keyReferenceQueue, value, valueReferenceQueue, weight, now);
  }

  public Node<K, V> newNode(
      Object keyReference, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
    return new PDRMS<>(keyReference, value, valueReferenceQueue, weight, now);
  }
}
