...
final class PDWMS<K, V> extends PDW<K, V> {
  int queueType;
  ...
  public Node<K, V> newNode(
      ...
      int weight,
      long now) {
    return new PDWMS<>(key, keyReferenceQueue, value, valueReferenceQueue, weight, now);
  }

  public Node<K, V> newNode(
      Object keyReference, V value, ReferenceQueue<V> valueReferenceQueue, int weight, long now) {
    return new PDWMS<>(keyReference, value, valueReferenceQueue, weight, now);
  }
}
